...
int ImGui::PlotExWithTips(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg,float middle_value,ImU32 up_color,ImU32 down_color,std::vector<std::string>& tips,std::string& hover_tip,std::vector<bool>& filter_flags,int* hover_index)
{
    ...
    if (values_count >= values_count_min)
    {
        ...
        //将idx_hovered赋值给hover_index
        *hover_index=idx_hovered;

        const float t_step = 1.0f / (float)res_w;
        const float inv_scale = (scale_min == scale_max) ? 0.0f : (1.0f / (scale_max - scale_min));//inv_scale是每一个单位值对应的百分比。比如最小值10，最大值20，那么每个单位就是1.0/(20-10)=0.1

        float v0 = values_getter(data, (0 + values_offset) % values_count);
        float t0 = 0.0f;

        //这里1.0f表示整个柱形图，计算到的t0.y表示当前数值的左上角点相比完整柱形的高度，1-t0.y才得到了实际数值柱形图高度。
        //那么ImSaturate返回值越大，表示的数值越大。v0是当前数值。
        //(v0 - scale_min) * inv_scale就是当前数值占了多少份。所以单纯将传入的Data里的数值都乘以一个倍数是不行的，起不到放大柱形图的效果。
        ImVec2 tp0 = ImVec2( t0, 1.0f - ImSaturate((v0 - scale_min) * inv_scale) );                       // Point in the normalized space of our target rectangle
        float histogram_zero_line_t = (scale_min * scale_max < 0.0f) ? (1 + scale_min * inv_scale) : (scale_min < 0.0f ? 0.0f : 1.0f);   // Where does the zero line stands
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            // pos0是左上角 pos1是右下角，pos0.y越小，说明柱形图越高，表示的值越大。pos0.y最小的是inner_bb.Min.y。如果一个值是0，那么pos0.y=inner_bb.Max.y。
            // 现在为了方便观看，要让小的值，柱形图更高一些，所以要让pos0.y更小一些。就要tp0.y更小些。
            // tp0.y是当前数值的左上角点相比完整柱形的高度，1减去它那么就得到了实际数值柱形图高度，那么之前肯定已经利用了最大值来计算百分比。
            if(tp0.y<1.0){
                tp0.y = 1 - (0.9 * (1 - tp0.y) + 0.1);
            }
            ...
            else if (plot_type == ImGuiPlotType_Histogram)
            {
                if(up_color>0 || down_color>0)
                {
                    ...
                    if(filter_flags.size()>v1_idx && filter_flags[v1_idx]==false) {
                        //如果没有通过过滤，灰色显示
                        col_base = GetColorU32(ImGuiCol_TextDisabled);
                    }
                }
...
void ImGui::PlotHistogramWithColor(const char* label, const float* values, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int stride,float middle_value,ImU32 up_color,ImU32 down_color,std::vector<std::string>& tips,std::string& hover_tip,std::vector<bool>& filter_flags,int* hover_index)
{
    ImGuiPlotArrayGetterData data(values, stride);
    PlotExWithTips(ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, (void*)&data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size,middle_value,up_color,down_color,tips,hover_tip,filter_flags,hover_index);
}
